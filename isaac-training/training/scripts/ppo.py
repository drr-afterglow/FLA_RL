...
class PPO(TensorDictModuleBase):
    def __init__(self, cfg, observation_spec, action_spec, device):
        ...
        self.cfg = cfg
        self.device = device
        # Feature extractor for static LiDAR
        ...
        # Feature extractor for dynamic obstacle LiDAR (3-channel panoramic image)
        dynamic_obstacle_network = nn.Sequential(
            nn.LazyConv2d(out_channels=8, kernel_size=[5, 3], padding=[2, 1]), nn.ELU(),
            nn.LazyConv2d(out_channels=16, kernel_size=[5, 3], stride=[2, 1], padding=[2, 1]), nn.ELU(),
            nn.LazyConv2d(out_channels=16, kernel_size=[5, 3], stride=[2, 2], padding=[2, 1]), nn.ELU(),
            Rearrange("n c w h -> n (c w h)"),
            nn.LazyLinear(128), nn.LayerNorm(128),
        ...
        self.feature_extractor = TensorDictSequential(
            TensorDictModule(feature_extractor_network, [("agents", "observation", "lidar_static")], ["_cnn_feature"]),
            TensorDictModule(dynamic_obstacle_network, [("agents", "observation", "lidar_dynamic")], ["_dynamic_obstacle_feature"]),
    ...
    def __call__(self, tensordict):
        ...
        # Transform body-frame velocity command to world frame using current attitude
        actions_body = (2 * tensordict["agents", "action_normalized"] * self.cfg.actor.action_limit) - self.cfg.actor.action_limit
        quat_w = tensordict["info", "drone_state"][..., 3:7]
        # Align action tensor batch dims with quaternion batch dims before rotation
        if actions_body.shape[:-1] != quat_w.shape[:-1]:
            actions_body = actions_body.reshape(*quat_w.shape[:-1], actions_body.shape[-1])
        actions_world = quat_rotate(quat_w, actions_body)
