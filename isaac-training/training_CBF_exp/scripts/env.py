...
class NavigationEnv(IsaacEnv):
    ...
    def __init__(self, cfg):
        ...
        # CBF parameters 
        self.cost_gamma = cfg.algo.safety.cost_gamma   # CBF 收敛系数
        self.r_safe = cfg.algo.safety.r_safe           # 安全半径
        ...
        self.lidar_hbeams = int(360/self.lidar_hres) #水平方向总的激光数量
        ...
        ray_caster_cfg = RayCasterCfg(
            ...
            offset=RayCasterCfg.OffsetCfg(pos=(0.0, 0.0, 0.0)),#lidar相对于无人机的位置偏移从，此处挂在于无人机的base_link上
            attach_yaw_only=True,#设置lidar只跟随无人机的偏航角旋转，与pitch和roll无关
        ...
        with torch.device(self.device):
            ...
            # 创建了目标位置张量、目标方向张量、高度范围张量和前一时刻无人机速度张量
    ...
    #仿真场景设计，无人机、地形、动态障碍物、lidar，在super().__init__()之后调用
    def _design_scene(self):
        ...
        #create static terrain accrording to the cfg
        terrain_importer = TerrainImporter(terrain_cfg)

        # Dynamic obstacle terrain
        ...
        for category_idx in range(cuboid_category_num + cylinder_category_num):
            ...
            # Spawn various sizes of dynamic obstacles 实际生成几何体
            if (category_idx < cuboid_category_num):
    ...
    def _compute_state_and_obs(self):
        ...
        self.lidar_scan = self.lidar_range - (
            (self.lidar.data.ray_hits_w - self.lidar.data.pos_w.unsqueeze(1)) #world，雷达击中位置-雷达位置
            .norm(dim=-1)
            .clamp_max(self.lidar_range)
            .reshape(self.num_envs, 1, *self.lidar_resolution)
        ) # lidar scan store the data that is range - distance and it is in lidar's local frame，lidar_scan是危险度信息而不是深度图
        ...
        # b. unit direction vector to goal 此时的目标方向向量是世界坐标系下的
        ...
        # final drone's internal states[unit goal direction(3), 2d distance(1), z distance(1), velocity in goal frame(3)]
        ...
        if (self.cfg.env_dyn.num_obstacles != 0):
            ...
            dynamic_collision_2d = closest_dyn_obs_distance_2d_collsion <= (closest_dyn_obs_width/2. + 0.3) #两个物体的中心距离小于两者半径之和时发生碰撞
            dynamic_collision_z = closest_dyn_obs_distance_zn_collision <= (closest_dyn_obs_height/2. + 0.3)#垂直距离小于等于高度安全距离则碰撞
        ...
        # g.reach goal 
        r_success = reach_goal.float() * 20.0

        # Final reward calculation
        if (self.cfg.env_dyn.num_obstacles != 0):
            self.reward = reward_vel*1.5 - 0.1 - penalty_smooth * 0.05 - penalty_height * 2.0 + r_success 
        else:
            self.reward = reward_vel*1.5 - 0.1 - penalty_smooth * 0.05 - penalty_height * 2.0 + r_success 
        ...
        self.terminated = below_bound | above_bound | collision | reach_goal #任务结束标志
    ...
    def _compute_reward_and_done(self):
        ...
        cbf_cost = self._compute_cbf_cost()
        self.stats["cbf_cost"] += cbf_cost
    ...
    # def _compute_cbf_cost(self):
    #     """
    #     [Final Version] 计算综合控制障碍函数 (CBF) 成本
    #     融合了：
    #     1. 动态障碍物 (基于 Python 缓存状态，与 Observation 对齐)
    #     2. 静态障碍物 (基于 LiDAR 击中点)
    #     """
    #     # --- 超参数设置  ---
    #     gamma = self.cost_gamma           # CBF 收敛系数 (越大越激进，越小越保守)
    #     r_safe_dyn = self.r_safe + 0.1    # 动态障碍物安全半径 (无人机半径 + 障碍物半径 + 缓冲)
    #     r_safe_stat = self.r_safe         # 静态障碍物安全半径 (墙壁不会动，可以稍小)

    #     # 1. 获取无人机当前物理状态 (Ground Truth from Sim)
    #     drone_pos_w = self.drone.pos[..., :3]  # (num_envs, 1, 3)
    #     drone_vel_w = self.drone.vel_w[..., :3] # (num_envs, 1, 3)

    #     # ==========================================
    #     # Part A: 动态障碍物 CBF (Dynamic Obstacles)
    #     # ==========================================
    #     if self.cfg.env_dyn.num_obstacles > 0:
    #         all_obs_pos = self.dyn_obs_state[..., :3] 
    #         all_obs_vel = self.dyn_obs_vel            

    #         # 1. 计算距离矩阵 (Broadcasting)
    #         dist_vec = drone_pos_w - all_obs_pos.unsqueeze(0)
    #         dists = torch.norm(dist_vec, dim=-1) 
            
    #         # 2. 找到最近的那个动态障碍物 (Min over obstacles)
    #         min_dists, min_idx = torch.min(dists, dim=1) # (num_envs,)
            
    #         # 3. 提取最近障碍物的状态
    #         nearest_obs_pos = all_obs_pos[min_idx] 
    #         nearest_obs_vel = all_obs_vel[min_idx] 

    #         # 4. CBF 公式计算: h = d_dot + gamma * (d - r_safe)
    #         # d_dot = (v_rel dot p_rel) / d
    #         rel_pos = drone_pos_w.squeeze(1) - nearest_obs_pos
    #         rel_vel = drone_vel_w.squeeze(1) - nearest_obs_vel
            
    #         # 单位方向向量 (指向无人机)
    #         unit_rel_pos = rel_pos / (min_dists.unsqueeze(-1) + 1e-6)
            
    #         # 相对速度在连线方向上的投影
    #         d_dot_dyn = torch.sum(rel_vel * unit_rel_pos, dim=-1, keepdim=True) # (num_envs, 1)
            
    #         # 计算 h 值
    #         h_dyn = d_dot_dyn + gamma * (min_dists.unsqueeze(-1) - r_safe_dyn)
            
    #         # ReLU 激活: 安全时(h>0) cost=0; 危险时(h<0) cost>0
    #         cost_dyn = torch.relu(-h_dyn)
    #     else:
    #         cost_dyn = torch.zeros((self.num_envs, 1), device=self.device)

    #     # ==========================================
    #     # Part B: 静态障碍物 CBF (Static via LiDAR)
    #     # ==========================================
    #     # 利用 LiDAR 的 Ray Hits 作为静态障碍物采样点
    #     # self.lidar.data.ray_hits_w: (num_envs, num_rays, 3)
        
    #     # 1. 计算向量: Drone -> Hit Point
    #     ray_hits = self.lidar.data.ray_hits_w
    #     vec_to_hits = ray_hits - drone_pos_w 
        
    #     # 2. 计算距离
    #     dists_stat = torch.norm(vec_to_hits, dim=-1) # (num_envs, num_rays)
        
    #     # 3. 掩码过滤: 忽略未击中(最大量程)的射线
    #     # 只有当 dist < range - epsilon 时，才视为有效障碍物
    #     valid_mask = dists_stat < (self.lidar_range - 0.1)
        
    #     # 4. 计算 d_dot (对于静态物体 v_obs = 0)
    #     unit_vec_to_hits = vec_to_hits / (dists_stat.unsqueeze(-1) + 1e-6)
        
    #     # 投影速度: 如果无人机飞向墙，v_drone 与 unit_vec_to_hits 点积为正
    #     # 但我们需要的是 "距离变化率"。飞向墙 = 距离减小 = 负值。
    #     # 所以 d_dot = - (v dot unit_vec)
    #     v_proj = torch.sum(drone_vel_w * unit_vec_to_hits, dim=-1)
    #     d_dot_stat = -v_proj
        
    #     # 5. 计算每条射线的 h 值
    #     # h = d_dot + gamma * (d - r_safe)
    #     h_stat_all = d_dot_stat + gamma * (dists_stat - r_safe_stat)
    #     safe_mask = (dists_stat > r_safe_stat) & (d_dot_stat > 0)
    #     h_stat_all[safe_mask] = float('inf')
    #     h_stat_all[~valid_mask] = float('inf')
        
    #     # 对于无效射线（空气），设为无穷大（绝对安全）
    #     h_stat_all[~valid_mask] = float('inf')
        
    #     # 6. 找到最危险的那根射线 (Min over rays)
    #     # 只有最危险的点决定了当前的 Cost
    #     h_stat_min, _ = torch.min(h_stat_all, dim=1, keepdim=True) # (num_envs, 1)
        
    #     cost_stat = torch.relu(-h_stat_min)

    #     # ==========================================
    #     # Part C: 综合输出
    #     # ==========================================
    #     # 取动态和静态中的最大威胁作为当前步的 Cost
    #     cbf_cost = torch.max(cost_dyn, cost_stat)
    #     cbf_cost = torch.clamp(cbf_cost, max=10.0)
    ...
    def _compute_cbf_cost(self):
        ...
        CBF 定义: h(x) = d - r_safe  (距离减安全距离)
        
        CBF 约束: ḣ + gamma*h ≥ 0  →  ḋ + gamma*(d - r_safe) ≥ 0
        
        Cost 函数 (指数):
            cost = exp(-2 * constraint) - 1,  when constraint < 0
                = 0,                          when constraint >= 0
        
        其中 constraint = ḋ + gamma*(d - r_safe)
        使用指数函数可以平滑地惩罚接近障碍物的行为,并且对多个障碍物的威胁进行加权聚合,区分2D/3D障碍物,考虑障碍物尺寸。
        """
        # CBF param
        gamma = self.cost_gamma                
        r_safe_drone = self.r_safe             
        r_safe_buffer_dyn = 0.2                
        r_safe_buffer_stat = 0.0               
        eps = 1e-6                            
        cost_alpha = 1.5 
        MAX_EXP_INPUT = 10.0      
        MAX_COST_SINGLE = 10.0                      
        
        # 聚合参数
        aggregation_temp = 0.5                 # Softmax 温度
        
        # uav state
        drone_pos = self.drone.pos[..., :3]      
        drone_vel = self.drone.vel_w[..., :3]    
        
        # --------------dynamic obstacle CBF caculate-------------
        if self.cfg.env_dyn.num_obstacles > 0:
            num_obs = self.cfg.env_dyn.num_obstacles
            num_3d_obs = num_obs // 2
            
            obs_pos = self.dyn_obs_state[:, :3]
            obs_vel = self.dyn_obs_vel
            obs_size = self.dyn_obs_size
            
            drone_pos_exp = drone_pos.expand(-1, num_obs, -1)
            drone_vel_exp = drone_vel.expand(-1, num_obs, -1)
            
            rel_pos = drone_pos_exp - obs_pos.unsqueeze(0)
            rel_vel = drone_vel_exp - obs_vel.unsqueeze(0)
            
            rel_pos_for_dist = rel_pos.clone()
            rel_vel_for_dist = rel_vel.clone()
            
            is_2d_obs = torch.zeros(num_obs, dtype=torch.bool, device=self.device)
            is_2d_obs[num_3d_obs:] = True
            
            rel_pos_for_dist[:, is_2d_obs, 2] = 0.0
            rel_vel_for_dist[:, is_2d_obs, 2] = 0.0
            
            dists = torch.norm(rel_pos_for_dist, dim=-1).clamp(min=eps)
            
            unit_rel_pos = rel_pos_for_dist / dists.unsqueeze(-1)
            d_dot = torch.sum(rel_vel_for_dist * unit_rel_pos, dim=-1)
            
            obs_radius = obs_size[:, 0] / 2
            r_safe_total = r_safe_drone + obs_radius + r_safe_buffer_dyn
            
            cbf_constraint_dyn = d_dot + gamma * (dists - r_safe_total.unsqueeze(0))
            
            exp_input_dyn = (-cost_alpha * cbf_constraint_dyn).clamp(max=MAX_EXP_INPUT)
            cost_per_obs = (torch.exp(exp_input_dyn) - 1).clamp(min=0, max=MAX_COST_SINGLE)
            
            in_range_mask = dists < self.lidar_range
            cost_per_obs = cost_per_obs * in_range_mask.float()
            
            cost_for_softmax = cost_per_obs.clamp(max=MAX_COST_SINGLE)
            max_cost = cost_for_softmax.max(dim=1, keepdim=True)[0]
            cost_shifted = (cost_for_softmax - max_cost) / (aggregation_temp + eps)
            weights = torch.softmax(cost_shifted, dim=1)
            
            cost_dyn = (weights * cost_per_obs).sum(dim=1, keepdim=True)
        else:
            cost_dyn = torch.zeros((self.num_envs, 1), device=self.device)
        
        # ---------------static obstacle CBF caculate-----------------------------
        ray_hits = self.lidar.data.ray_hits_w
        lidar_pos = self.lidar.data.pos_w
        
        vec_to_hits = ray_hits - lidar_pos.unsqueeze(1)
        
        # [FIX] 计算距离，将 inf 替换为最大范围 (4m)
        dists_stat = torch.norm(vec_to_hits, dim=-1)
        dists_stat = torch.where(torch.isinf(dists_stat), torch.tensor(self.lidar_range, device=self.device), dists_stat)
        dists_stat = dists_stat.clamp(min=eps)
        
        valid_mask = dists_stat < (self.lidar_range - 0.1)
        
        # [FIX] 对于 inf 射线，vec_to_hits 也是 inf，需要处理
        # 将 inf 的 vec_to_hits 设为一个安全的默认值
        vec_to_hits = torch.where(torch.isinf(vec_to_hits), torch.zeros_like(vec_to_hits), vec_to_hits)
        
        unit_vec_to_hits = vec_to_hits / dists_stat.unsqueeze(-1)
        drone_vel_for_stat = drone_vel.squeeze(1).unsqueeze(1)
        v_proj = torch.sum(drone_vel_for_stat * unit_vec_to_hits, dim=-1)
        d_dot_stat = -v_proj
        
        r_safe_stat = r_safe_drone + r_safe_buffer_stat
        
        cbf_constraint_stat = d_dot_stat + gamma * (dists_stat - r_safe_stat)
        
        exp_input_stat = (-cost_alpha * cbf_constraint_stat).clamp(max=MAX_EXP_INPUT)
        cost_per_ray = (torch.exp(exp_input_stat) - 1).clamp(min=0, max=MAX_COST_SINGLE)
        cost_per_ray = cost_per_ray * valid_mask.float()

        cost_for_softmax_stat = cost_per_ray.clamp(max=MAX_COST_SINGLE) + eps * valid_mask.float()
        max_cost_stat = cost_for_softmax_stat.max(dim=1, keepdim=True)[0]
        cost_shifted_stat = (cost_for_softmax_stat - max_cost_stat) / (aggregation_temp + eps)
        weights_stat = torch.softmax(cost_shifted_stat, dim=1)
        
        cost_stat = (weights_stat * cost_per_ray).sum(dim=1, keepdim=True)
        
        # ------------------final cost combine--------------------
        cost_dyn = cost_dyn.clamp(max=5.0)
        cost_stat = cost_stat.clamp(max=5.0)
        cbf_cost = cost_dyn + cost_stat
        cbf_cost = cbf_cost.clamp(max=10.0)
